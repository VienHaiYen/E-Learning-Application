...
import 'package:elearning_app/screens/video_call/video_call.dart';
...
import 'package:elearning_app/services/booking_service.dart';
...
class _ScheduleItemState extends State<ScheduleItem> {
  String _convertClassTime() {
    ...
    result += DateFormat.Hm().format(DateTime.fromMillisecondsSinceEpoch(
        widget.bookingInfo.scheduleDetailInfo!.startPeriodTimestamp ?? 0));
    result += ' - ';
    result += DateFormat.Hm().format(DateTime.fromMillisecondsSinceEpoch(
        widget.bookingInfo.scheduleDetailInfo!.endPeriodTimestamp ?? 0));
    return result;
  }
  ...
  Future<String> _handleCancelClass(AuthProvider authProvider) async {
    ...
    final result = await BookingService.cancelBookedClass(
      scheduleDetailIds: [widget.bookingInfo.id ?? ''],
      token: token,
    );
    return result;
  }
  ...
  Widget build(BuildContext context) {
    ...
    final String meetingToken =
        widget.bookingInfo.studentMeetingLink?.split('token=')[1] ?? '';
                      ...
                      if (_isTimeToJoin()) {
                        ...
                        if (result) {
                          ...
                          Navigator.push(context, MaterialPageRoute(
                            builder: (context) {
                              final start = widget.bookingInfo
                                  .scheduleDetailInfo!.startPeriodTimestamp!;
                              return VideoCall(startTimestamp: start);
                            },
                          ));
                        }
                      }
